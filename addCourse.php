<?php
    ...
    if(isset($_GET['id']) && isset($_GET['status'])){
        ...
        if($courseStatus == "Deactivate"){
            $sqlCourse = "update course set status='Inactive' where id=$courseId";
            $con->query($sqlCourse);
        }else if($courseStatus == "Activate"){
            $sqlCourse = "update course set status='Active' where id=$courseId";
            $con->query($sqlCourse);
        }
...
<body>
        ...
        <input type="radio" name="same" id="r2">
            <div class="container">
                ...
                <ul>
                    ...
                    <?php
                            ...
                            echo "<li><a  class='activeLink' href='addCourse.php'>コースを追加</a></li>";
            ...
            <div class="addCourse">
                <button class="add-course-btn" onclick="ShowForm()">Add Course</button></br>
                <form action="addCourse.php" method="post">
                    ...
                    <table id="courseList">
                        ...
                        <?php
                            ...
                            if($courseCnt == 0){
                                echo "<tr>";
                                    echo "<td colspan='4'>コースがありません。</td>";
                                echo "</tr>";
                            }else{
                                do{
                                    echo "<tr>";
                                        ...
                                        echo "<td>{$rowsCourse['course_name']}</td>";
                                        echo "<td>{$rowsCourse['status']}</td>";
                                        $courseStatus = $rowsCourse['status'];
                                        $courseStatus = ($courseStatus == "Active" ? "Deactivate" : "Activate");
                                    if($courseStatus == "Active"){
                                        echo "<td><button><a href='addCourse.php?id=" . $rowsCourse['id'] . "&status=" . $courseStatus . "'>$courseStatus</a></button></td>";
                                    }else{
                                        echo "<td><button><a href='addCourse.php?id=" . $rowsCourse['id'] . "&status=" . $courseStatus . "'>$courseStatus</a></button></td>";
                                    }
                    ...
                    <a href="addCourse.php"><button class="fas fa-times" id="times2""></button></a></br></br>

                    <table id="addCourseForm">
                        <form action="addCourse.php" method="POST">
                            <tr>
                                <th><label for="course" class="course">Course:</label></th>
                                <td><input type="text" id="course" name="course"></br></td>
                            </tr>
                            <tr>
                                <th><label for="courseName" class="courseName">Course Name:</label></th>
                                <td><input type="text" id="courseName" name="courseName"><br></td>
                            </tr>
                            
                            <tr>
                                <td></td>
                                <td><input type="button" onclick="reset()" value="Reset">&nbsp;&nbsp;
                                <input type="submit" name="addCourse" value="Submit"></td>
                            </tr>
